...
const EditDepartmentAlert = () => {
    ...
    const handleRecipientSelection = (employee) => {
        const isSelected = alertData.recipients.map(item => item.employeeID).includes(employee.employeeID);
        const newRecipients = isSelected ? alertData.recipients.filter(item => item.employeeID !== employee.employeeID) : [...alertData.recipients, employee];
        setAlertData(prevData => ({
            ...prevData,
            recipients: newRecipients
        }));
        setAllSelectedEmployees(newRecipients);
    };
    ...
    const updateAlert = async (e) => {
        ...
        formData.append("recipients", JSON.stringify(alertData.recipients.map(item => item.employeeID)));
        ...
        try {
            ...
            alertData.attachments = update_response.data.attachments;
                    ...
                    <div className="bg-white p-2 flex flex-col gap-2 border border-gray20 rounded-[10px] mt-1">
                        ...
                        <ul className={`space-y-2 overflow-y-scroll ${employees.length > 0 ? "h-48" : "h-auto"}`}>
                            {filteredEmployees.map(emp => {
                                const isSelected = alertData.recipients.map(item => item.employeeID).includes(emp.employeeID);
                                return (
                                    <li key={emp.employeeID} onClick={() => handleRecipientSelection(emp)} 
                                        ...
                                        <div className="flex items-center gap-2">
                                            <input
                                                type="checkbox"
                                                checked={alertData.recipients.map(item => item.employeeID).includes(emp.employeeID)}
